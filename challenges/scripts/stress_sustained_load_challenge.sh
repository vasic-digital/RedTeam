...
echo "=== RedTeam Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: REDTEAM_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== RedTeam Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== RedTeam Stress Challenge: PASSED ==="
